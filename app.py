...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///C:/Users/anany/Desktop/myproject/collegeroots/blog.db'
app.config['SQLALCHEMY_BINDS'] = {'teo':'sqlite:///C:/Users/anany/Desktop/myproject/collegeroots/teo.db',
                                   'reg':'sqlite:///C:/Users/anany/Desktop/myproject/collegeroots/reg.db'}
...
def addpost1():
    ...
    if subtitle =="MMMUT":
      post = teo(title=title, subtitle=subtitle, author=author, content=content, date_posted=datetime.now())
    else:
...
def login_validation():
 ...
 emaildata = cc.execute("SELECT email FROM reg WHERE email=:email" , {"email":email}).fetchone()
 passworddata = cc.execute("SELECT password FROM reg WHERE email=:email", {"email":email}).fetchone()
